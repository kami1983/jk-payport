...
<table>
    ...
    <tr>
        ...
        <td>
            ...
            <input type='submit' value="do_response" onclick="$('#id_json_str').val('<?php echo base64_encode($payportmentobj->do_response); ?>');" />
            <?php endif; ?>
        </td>
        <td>
            <?php if('' != $payportmentobj->payment_json): ?>
            <input type='submit' value="payment_json" onclick="$('#id_json_str').val('<?php echo base64_encode($payportmentobj->payment_json); ?>');" />
            <?php endif; ?>
        </td>
        <td>
            <?php if('' != $payportmentobj->get_json): ?>
            <input type='submit' value="get_json" onclick="$('#id_json_str').val('<?php echo base64_encode($payportmentobj->get_json); ?>');" />
            <?php endif; ?>
        </td>
        <td>
            <?php if('' != $payportmentobj->post_json): ?>
            <input type='submit' value="post_json" onclick="$('#id_json_str').val('<?php echo base64_encode($payportmentobj->post_json); ?>');" />
            <?php endif; ?>
        </td>
    </tr>
    <?php endforeach; ?>
